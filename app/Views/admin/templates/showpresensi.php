    ...
    <div class="container-fluid px-1 mt-4">
        <div class="row mx-0">
            ...
            <div class="col">
                <div class="row mx-0" style="border-bottom: 1px solid gray; min-width: 270px;">
                    ...
                    <a href="<?=base_url()?>/admin/arsip/jurnal/show/<?=$guru['id']?>/0/0" id="jrnlmenu" class="col text-center menu-link my-0 px-1 py-1"><span>Jurnal Mengajar Guru</span></a>
                </div>
            </div>
        </div>
        <div class="row mx-0 mb-3 py-0">
            <div class="col py-1 pb-3 mt-2 mx-2 mt-2" style="box-shadow: 1px 1px 8px 2px rgba(0,0,0,0.2) !important; min-width: 300px;
                ...
                ">
                <div class="row">
                    <div class="col fs-5 py-2 text-white fw-bold"><i class="fas fa-list-ol text-white mr-2"></i>Presensi Siswa</div>
                </div>
                <div class="row mx-0 text-white text-center">
                    ...
                    <table class="col">
                        ...
                        <tr>
                            <td class="text-center">
                                <span class="fw-bolder badge fs-5 mx-2 mb-2" style="background-color: rgba(12,180,220,1); min-width: 50px;"><?="NUPTK / NIP. " . $guru['nip']?></span>
                            </td>
                        </tr>
                    </table>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        dt = "kelas="+$("#pilihkelas").val()+"&idguru=<?=$guru['id']?>";
        $.ajax({
			type: 'POST',
			url: "/admin/templates/getmapelrange",
			data: dt,
        ...
        $('#pilihkelas').on('change', function() {
			...
			type: 'POST',
			url: "/admin/templates/getmapelrange",
			data: "kelas="+kelas+"&idguru=<?=$guru['id']?>",
			success: function(hasil) {
				$("#pilihmapel").html(hasil);
			}
    	    });
        ...
        $('#pilihBulan').on('change', function() {
            ...
            window.location.href = "<?= base_url() ?>/admin/arsip/presensi/show/" + "<?=$guru['id']?>/" + bulan + "/" + tahun;
        });
        ...
        $('#cetakRange').on('click', function() {
            ...
            window.location.href = "<?= base_url() ?>/admin/arsip/presensi/cr/" + "<?=$guru['id']?>/" + bln1 + "/" + bln2 + "/" + kelas + "/" + mapel;
        });
