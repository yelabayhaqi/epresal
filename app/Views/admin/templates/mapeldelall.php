...
$this->db->table('kelas_has_mapel')->where('kelas',$kelas)->delete();
$mapel = $this->db->table('kelas_has_mapel')->select()
->join('mapel','kelas_has_mapel.mapel = mapel.id', 'left')
->where('kelas_has_mapel.kelas', $kelas)->orderBy('nama_mapel','asc')
->get()->getResultArray();
if($mapel != NULL){?>
<div class="mx-0">
    <div class="list-group list-group-flush ml-2 pr-0">
        ...
        <?php $kat = $mp['kategori']; }?>
            <div class="list-group-item text-dark fs-6 fw-normal px-1 px-1 ml-3 me-5">
                <?=$mp['nama_mapel']?>
